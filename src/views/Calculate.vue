  ...
  <v-container fluid>
    ...
    <v-row v-else>
      ...
      <v-col v-if="money > 0" cols="12" md="12" sm="12">
        <h1>
          <v-icon size="30">attach_money</v-icon>Your <b class="grey--text">expected return</b> is: {{ ReturnOfInvestment }}
        </h1>
        <h2>
          <v-icon size="30" left>access_time</v-icon>Duration: {{ duration }} months
        </h2>
        <h2>
          <v-icon size="30" left>monetization_on</v-icon>Interest paid: {{ Math.round(interest*100*100)/100}} %
        </h2>
        <h2>
          <v-icon size="30" left>dashboard</v-icon>Your chosen bond: {{ name }}
        </h2>
      </v-col>
                ...
                <v-list-item three-line>
                  <v-list-item-content>
                    <v-list-item-title class="mb-4 mt-4">
                      <h3>
                        <v-icon>schedule</v-icon><b><span> Duration :</span> {{bond.duration_months}} months</b>
                      </h3>
                    </v-list-item-title>
                    <v-list-item-title class="headline mb-4 mt-4">
                      <h3 id="bondname">{{bond.name}}</h3>
                    </v-list-item-title>
                  </v-list-item-content>

                  <v-list-item-avatar size="50" color="black">
                    <v-icon color="white" size="30">attach_money</v-icon>
                  </v-list-item-avatar>
                </v-list-item>
