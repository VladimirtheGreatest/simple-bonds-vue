...
<div class="Investor pt-10">
  ...
  <v-container fluid>
        ...
        <v-list shaped class="mx-auto" max-width="500" tile outlined elevation="12">
          <v-list-item>
            ...
            <v-list-item-content>
              <v-list-item-icon>
                <div>
                  <v-list-item-title class="mb-5">
                    <v-icon>person</v-icon><span class="font-weight-light">{{ investment.first_name }} {{ investment.last_name }}</span>
                  </v-list-item-title>
                </div>
              </v-list-item-icon>
              ...
              <v-list-item-title class="mb-5">
                <v-icon>attach_money</v-icon><span class="font-weight-bold">Investment Amount:</span> £{{ investment.amount / 100 }}
              </v-list-item-title>
              <v-list-item-title class="mb-5">
                <v-icon left>money</v-icon><span class="font-weight-bold">Investment Type:</span> {{ investment.type }}
              </v-list-item-title>
              <v-list-item-title class="mb-5">
                <v-icon left>access_time</v-icon><span class="font-weight-bold">Created at:</span> {{ investment.created_at }}
              </v-list-item-title>
              <v-list-item-title v-if="investment.status !== 'pending'" class="mb-5">
                <v-icon left>timelapse</v-icon><span class="font-weight-bold">Last update:</span> {{ investment.updated_at}}
              </v-list-item-title>
              ...
              <v-btn :id="investment.id" @click="deleteInvestment(investment.id)" class="red mt-4" v-else>
                <!-- this will generate id for each investment, I will then use that in the API request to delete chosen investment-->
    ...
    <h1 class="subheading black--text pt-10">
      <v-icon size="30" left>monetization_on</v-icon>Make your <b class="grey--text">money</b> grow
    </h1>
        ...
        <v-row class="mb-10">
          <v-col cols="12">
            <div class="flex-center">
              <v-layout>
                <v-flex>
                  <v-text-field label="How much do you want to invest?" value="0.00" prefix="£" v-model="money" clearable outlined size="100"></v-text-field>
                </v-flex>
              </v-layout>
            </div>
          </v-col>
          ...
          <v-col cols="12">
            <h1>
              <v-icon size="30">attach_money</v-icon>Your investment: <span v-if="money > 0"> £{{ money }}</span>
            </h1>
          </v-col>

          <v-col cols="12">
            <h1>
              <v-icon size="30" left>access_time</v-icon>Interest paid on: {{ type }}
            </h1>
          </v-col>
        </v-row>
    ...
    <v-btn class="mt-10" x-large rounded dark to="/Investors">
      <v-icon size="40">arrow_back</v-icon>Back
    </v-btn>
  </v-container>
</div>
</template>
